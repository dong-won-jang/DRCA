...
class ResidualGroup(nn.Module):
    def __init__(self, conv, in_feat, n_feat, kernel_size, reduction, act, res_scale, n_resblocks):
        ...
        modules_body = []
        modules_body.append(RCABP(conv, in_feat, n_feat, kernel_size, reduction, bias=True, bn=True, act=nn.ReLU(True), res_scale=1))
        for i in range(n_resblocks-1):
            modules_body.append(RCAB(conv, n_feat, kernel_size, reduction, bias=True, bn=True, act=nn.ReLU(True), res_scale=1))
        self.body = nn.Sequential(*modules_body)
...
class DRCA(nn.Module):
    def __init__(self, args, conv=common.default_conv):
        ...
        for i in range(n_resgroups):
            modules_body.append(ResidualGroup(conv, (i+1)*n_feats, n_feats, kernel_size, reduction, act=act, res_scale=args.res_scale, n_resblocks=n_resblocks))
